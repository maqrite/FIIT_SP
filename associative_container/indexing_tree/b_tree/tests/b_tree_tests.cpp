...
bool compare_obtain_results(std::vector<tvalue> const &expected,
...
template <typename tkey, typename tvalue> struct test_data {
  ...
  test_data(size_t d, size_t i, tkey k, tvalue v)
      : depth(d), index(i), key(k), value(v) {}
};
...
bool infix_const_iterator_test(
  ...
  for (auto const &item : expected_result) {
    ...
    if (it->first != item.key || it->second != item.value ||
        it.depth() != item.depth || it.index() != item.index) {
      return false;
    }
...
TEST(bTreePositiveTests, test0) {
  ...
  B_tree<int, std::string, std::less<int>, 1024> tree(std::less<int>(),
                                                      nullptr);
...
TEST(bTreePositiveTests, test7) {
  std::vector<std::string> expected_result = {"g", "d", "e", " ",
                                              "l", "a", "b", "y"};
...
TEST(bTreePositiveTests, test8) {
  std::vector<std::string> expected_result = {"y", "l", "a", "g",
                                              "k", "b", "c", "h"};
