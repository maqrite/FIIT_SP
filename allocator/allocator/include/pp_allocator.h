...
template <typename T> struct pp_allocator {
private:
...
public:
  ...
  pp_allocator(std::pmr::memory_resource *mem =
                   std::pmr::get_default_resource()) noexcept;

  template <class U> pp_allocator(const pp_allocator<U> &other) noexcept;
  ...
  void deallocate_bytes(void *p, size_t bytes = 1,
...
template <typename T>
bool operator==(const pp_allocator<T> &lhs,
                const pp_allocator<T> &rhs) noexcept {
...
template <typename T>
void pp_allocator<T>::deallocate_bytes(void *p, size_t bytes,
                                       size_t alignment) {
...
template <typename T> template <class U> void pp_allocator<T>::destroy(U *p) {
  p->~U();
}
...
void pp_allocator<T>::construct(U *p, Args &&...args) {
  std::__uninitialized_construct_using_allocator(p, *this,
                                                 std::forward<Args>(args)...);
}

template <typename T> void pp_allocator<T>::deallocate(T *p, size_t n) {
  _mem->deallocate(p, n * sizeof(T), alignof(T));
}

template <typename T> T *pp_allocator<T>::allocate(size_t n) {
  return reinterpret_cast<T *>(_mem->allocate(n * sizeof(T), alignof(T)));
}

template <typename T>
template <typename U>
pp_allocator<T>::pp_allocator(const pp_allocator<U> &other) noexcept
    : _mem(other.resource()) {}
